...
void tui::render::draw_text(vec2d start, std::u32string_view text) {
    tui::render::write(current_matrix(), start, text);
}

void tui::render::draw_text_styled(vec2d start, std::u32string_view text, Color text_col, Color bg_col, text_flags flags) {
    tui::render::write_styled(current_matrix(), start, text, text_col, bg_col, flags);
}
